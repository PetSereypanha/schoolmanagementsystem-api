...
export class ResetPayload {
    ...
    @ApiProperty({ description: 'Token for reset password', required: true , example: 'xdadaf1231sss'})
    @IsString()
    token: string;

    @ApiProperty({ description: 'Password of the user', minLength: 8 ,required: true, example: 'Password@123'})
    @MinLength(8, {
